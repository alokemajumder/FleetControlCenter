...
- Causality explorer for tracing file/tool references across sessions
...
- CSP nonces (per-request), security headers, rate limiting, request timeouts
- Automatic secret redaction in event payloads (key-name and Bearer token detection)
- Zero-trust action sandbox with command/path allowlists and symlink resolution
...
- Sandbox enforcement with command/path allowlists
...
- Demo data generator (30 days, 3 nodes, 210 sessions)
